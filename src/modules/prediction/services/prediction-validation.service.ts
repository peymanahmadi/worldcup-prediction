...
//    */
//   async validatePrediction(groups: { [key: string]: string[] }): Promise<void> {
...
//         `Missing groups: ${missingGroups.join(', ')}. All 12 groups (A-L) are required.`,
//       );
//     }
...
//         throw new BadRequestException(
//           `Group ${group} must be an array of team UUIDs`,
//         );
//       }
//       if (teams.length !== 4) {
//         throw new BadRequestException(
//           `Group ${group} must have exactly 4 teams. Found: ${teams.length}`,
//         );
//       }
//     }

//     // 3. Collect all team UUIDs
//     const allTeamIds: string[] = [];
//     for (const group of requiredGroups) {
//       allTeamIds.push(...groups[group]);
//     }

//     // 4. Check if total is 48 teams
//     if (allTeamIds.length !== 48) {
//       throw new BadRequestException(
//         `Total teams must be 48 (12 groups × 4 teams). Found: ${allTeamIds.length}`,
//       );
//     }
...
//         (id, index) => allTeamIds.indexOf(id) !== index,
//       );
//       throw new BadRequestException(
//         `Duplicate teams found: ${[...new Set(duplicates)].join(', ')}. Each team can only be used once.`,
//       );
//     }

//     // 6. Verify all team UUIDs exist in database
//     const existingTeams = await this.teamRepository.find({
//       where: { id: In(allTeamIds) },
//       select: ['id'],
//     });

//     if (existingTeams.length !== 48) {
//       const existingIds = existingTeams.map((t) => t.id);
//       const invalidIds = allTeamIds.filter((id) => !existingIds.includes(id));
//       throw new BadRequestException(
//         `Invalid team UUIDs found: ${invalidIds.slice(0, 5).join(', ')}${
//           invalidIds.length > 5 ? '...' : ''
//         }`,
//       );
//     }
...
//     this.logger.debug(
//       'Prediction validation passed: 12 groups, 4 teams each, 48 total',
//     );
//   }

//   /**
//    * Validate if user can submit/edit prediction
//    */
//   validateCanSubmit(isFinalized: boolean): void {
//     if (isFinalized) {
//       throw new BadRequestException(
//         'Prediction is already finalized and cannot be modified',
//       );
//     }
//   }
// }
